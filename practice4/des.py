...
from Crypto.Cipher import DES #DES Algorithm implementation
import os #Play with the filenames

#des module for encrypt/decrypt information
def desCipher(key,data,mode,function,iv):
	#We don't worry 'bout the initial vector if we are using ECB
	if(mode=='ECB'):		tempCipher = DES.new(toUtf8(padKeyDes(key)), DES.MODE_ECB)
	elif(mode=='OFB'):		tempCipher = DES.new(toUtf8(padKeyDes(key)), DES.MODE_OFB, toUtf8(padIV(iv,'DES')))
	elif(mode=='CFB'):		tempCipher = DES.new(toUtf8(padKeyDes(key)), DES.MODE_CFB, toUtf8(padIV(iv,'DES')))
	elif(mode=='CBC'):		tempCipher = DES.new(toUtf8(padKeyDes(key)), DES.MODE_CBC, toUtf8(padIV(iv,'DES')))
	elif(mode=='CTR'):		tempCipher = DES.new(toUtf8(padKeyDes(key)), DES.MODE_CTR, nonce=b'', initial_value=toUtf8(padIV(iv,'DES')))
	return tempCipher.encrypt(pad(data,'DES'))	if (function=='Encrypt') else  tempCipher.decrypt(pad(data,'DES')) 
...
def pad(data,mode):
	length	=	(DES.block_size - len(data)) % DES.block_size
...
def EncryptFile(mode,key,iv):
		...
		print(mode)
		#cbc mode
		image_array =  desCipher(key,image_array,mode,'Encrypt',iv)
		#We save the new image
		Image.frombytes("RGB", image.size, image_array, "raw", "RGB").save("encrypt"+"_"+filename+"_"+mode+".bmp",format='BMP')

		messagebox.showinfo("Success","Your file has been encrypted with DES-"+mode+" mode succesfully!")
#Encrypt image
def DecryptFile(mode,key,iv):
		...
		if(len(slash)>1):
			...
			x=slash[0]
		print(mode)
		image_array =  desCipher(key,image_array,mode,'Decrypt',iv)
		#We save the new image
		Image.frombytes("RGB", image.size, image_array, "raw", "RGB").save("decrypt"+"_"+x+"_"+mode+".bmp",format='BMP')
		messagebox.showinfo("Success","Your file has been Decrypted with DES-"+mode+" mode succesfully!")

if __name__ == "__main__":
	root = Tk()
	root.geometry('600x300')
	#This code is mine mine mineeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
	Title = root.title( "DES Operation Modes, by Alejandro Reyes")
