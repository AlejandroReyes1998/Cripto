...
public class encrypt {
    ...
    public void swap32() {
        ...
        for(int i=0; i<4;i++){
            for(int j=0; j<8; j++){
                LeftS[i][j]=Integer.toString(Right[ind]);
                RightS[i][j]=Integer.toString(Left[ind]);
                ind++;
            }
        }
        arrayprinter.printarray(LeftS,"Left Part");
...
//        System.out.println("PlainTETX: "+ PlainText.substring(start,start+9));
        ...
        for(int f=0; f<PlainText.length()/8; f++){
            ...
            while(Round<=16) {
                ...
                int chooser=num_Left[index_chooser];//////////////num to forward
            ...
            swap32();////se imprimen al reves cambiar
            
            contancate();
            System.out.println("/nBit Swap :");
            for(int i=0;i<64;i++) {System.out.print(Block64[i]);}
    ...
    public String getEncryption(){return finalEncry;}

    public String getBinCi(){return binCi;}
