		...
		if not outputName:
			...
			if not publicKey:
				...
				try:
					...
					ct_bytes = cipher_aes.encrypt(pad(self.file_contents, AES.block_size))
					iv = b64encode(cipher_aes.iv).decode('utf-8')
					ct = b64encode(ct_bytes).decode('utf-8')
					
					file_out.write(enc_session_key+b'\n_AES_\n'+iv.encode()+b'\nSECRET\n'+ct.encode())
			...
			if not privateKey:
				...
				with open(privateKey, "rb") as priv_key:
					try:
						...
						enc_session_key= self.file_contents.split("\n_AES_\n".encode(),1)[0]
						OTRO= self.file_contents.split("\n_AES_\n".encode(),1)[1]

						# Obtaining the contents from the file
